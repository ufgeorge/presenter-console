...
| 影片播放 | Notes 寫 `<video>檔名</video>`（與簡報同資料夾），手機出現「播放影片」按鈕，以系統播放器前景播放，可暫停 / 繼續 |
...
在講者備註中寫入 `<video>檔名</video>` 標籤（影片檔放在**簡報同一個資料夾**），手機上會出現「▶ 播放影片」按鈕：

1. 切到含 `<video>` 標籤的投影片 → 手機顯示「播放影片」按鈕（多個標籤可下拉選擇播放哪一支）
2. 點按鈕 → 電腦以**系統預設播放器**開啟影片並帶到前景（PowerPoint / Open Design 皆支援）
3. 手機有「⏸ 暫停 / ▶ 繼續」按鈕，可暫停後口頭說明再繼續
4. 支援寫法：`<video>demo.mp4</video>`（一行內、含檔名與副檔名）；建議標籤自成一行

> ⚠️ 注意：標籤若跨行或未閉合，內文會連進講稿（已自動過濾檔名不完整的標籤）；影片檔名要與實際檔案完全一致（含副檔名）。
